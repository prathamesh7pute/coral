    ...
    if (options.page) {
      if (options.sort) {
        Model.find(options.filter)
          ...
          .limit(options.perPage).execFind(cb);
      } else {
        Model.find(options.filter).skip(options.page * options.perPage).limit(options.perPage).execFind(cb);
      }
    } else if (options.skip && options.limit) {
      if (options.sort) {
        Model.find(options.filter).sort(options.sort).skip(options.skip).limit(options.limit).execFind(cb);
      } else {
        Model.find(options.filter).skip(options.skip).limit(options.limit).execFind(cb);
      }
    } else if (options.findAll) {
      if (options.sort) {
        Model.find(options.filter).sort(options.sort).execFind(cb);
      } else {
        Model.find(options.filter, cb);
      }
    } else {
      cb(null, []);
    }
